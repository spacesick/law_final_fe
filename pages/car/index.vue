  ...
  <div class="m-auto text-center w-fit min-w-[20rem]">
    ...
    <div v-if="productTable" class="flex flex-col gap-4 py-4 justify-center">
      <div
        ...
        :key="product.id"
        class="border-2 rounded-md shadow-lg px-4 py-6 transition-all hover:-translate-y-1"
      >
        <div class="flex flex-col md:flex-row text-left gap-8">
          <nuxt-link
            :to="'/car/' + product.id"
          ...
          >
            <img
              ...
              alt="car"
              class="m-auto max-h-[18rem] max-w-[32rem] min-w-[20rem] h-full w-fit object-cover rounded-lg"
            />
        ...
        <div class="flex w-full justify-start">
          <button
            class="inline-flex justify-center py-2 px-4 mt-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-700 border-blue-800 hover:bg-blue-800 transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500"
            @click="onMessageUser(product.owner)"
          >
            Chat seller
          </button>
        </div>
    ...
    <p v-else class="bg-red-300 font-light p-2 my-[12px] text-left">
      Could not fetch list of products.
    </p>
  </div>
</template>
...
export default {
  ...
  methods: {
    onMessageUser(user) {
      socket.emit("new messaged username", user);
    }
  },
};
</script>
