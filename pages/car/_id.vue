  ...
  <div class="m-auto text-center max-w-[60rem] px-2">
    ...
    <div v-if="productTable" class="flex flex-col gap-4 py-4">
      <div
        ...
        :key="product.id"
        class="border-2 rounded-md shadow-lg px-4 py-6 flex flex-col justify-center text-left gap-8"
      >
        <div class="flex-1 items-center">
          <img
            v-if="product.image_url"
            :src="product.image_url"
            class="m-auto max-h-[36rem] max-w-[48rem] h-fit w-full object-cover rounded-md"
          />
        ...
        <form class="flex flex-col justify-center gap-[inherit]">
          <div class="flex justify-center gap-4 text-center">
            <div>
              ...
              <input
                v-model="start_date"
                type="date"
                class="m-auto border-2 rounded-md"
              />
            </div>
            <div>
              <p><b>Finish date:</b></p>
          ...
          <button
            class="m-auto w-fit h-fit py-2 px-5 font-bold bg-green-500 border-4 border-green-400 rounded-md"
            title="Order"
            @click.prevent="orderProduct(index, product.owner, product.price)"
          >
            Order Now
          </button>
        </form>
...
export default {
  ...
  methods: {
    orderProduct(index, receiver, amount) {
      // alert(receiver);
      // alert(this.$store.state.auth.username);
      // alert(amount);
      const orderer = this.$store.state.auth.username;
      axios
        .post(
          process.env.ORCHESTRATOR_ENDPOINT + 'order',
