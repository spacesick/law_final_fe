<template>
  <form class="m-auto text-center max-w-[60rem] px-2 flex flex-col gap-2">
    ...
    <div class="text-left">
      <label class="flex flex-col gap-2 my-2 text-sm font-medium text-gray-900"
        >Vehicle Image
      ...
      <CInput
        v-model="no_of_seaters"
        label="Num of Seaters"
        type="text"
      ></CInput>
      ...
      <label class="block my-2 text-sm font-medium text-gray-900 text-center"
        >Availability</label
      >
      ...
      <div class="text-center py-4">
        <button
          class="py-2 px-5 font-bold bg-green-500 border-4 border-green-400 rounded-md"
          title="Add"
...
export default {
  ...
  methods: {
    ...
    addProduct() {
      ...
      if (!token) {
        this.error = 'Please logout of your account and login again.'
        return;
      }
      ...
      axios
        .post(process.env.PRODUCT_ENDPOINT + 'product/', formData, {
          headers: {
